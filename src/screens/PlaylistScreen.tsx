...
export default function PlaylistScreen() {
  ...
  useEffect(() => {
    const fetchSongs = async () => {
      try {
        const res = await musicApi.getPlaylist(playlistId, 0, 50);
        const ok = res?.success === true;
        const list = ok ? res.data?.songs || res.data || [] : [];
        setSongs(list);
      } catch (e) {
        console.error('Playlist songs error', e);
      } finally {
        setLoading(false);
      }
    };
  ...
  const renderItem = ({ item, index }: { item: Song; index: number }) => {
    ...
    const imageUrl = getImageUrl(item.image, '150x150') || playlistImage || '';
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleWrap}>
          {playlistImage ? <Image source={{ uri: playlistImage }} style={styles.headerAvatar} /> : null}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              <Text style={styles.subtitle}>No songs found for this playlist.</Text>
            </View>
          }
        />
      )}
...
const styles = StyleSheet.create({
  ...
  headerAvatar: { width: 32, height: 32, borderRadius: 6, marginRight: spacing.sm, backgroundColor: colors.surface },
